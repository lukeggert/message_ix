...
SUM(year2$( seq_period(year2,year) ), K(node_active, year2) * (1 - depr(node_active))**duration_period(year2) + duration_period(year2) * (K(node_active, year2) * ((1 + INTEREST(node_active, year2))**duration_period(year2) - 1) 
+ labor(node_active, year2) * WAGE(node_active, year2) 
- eneprice(node_active, 'rc_spec', year2)/1000 * labor(node_active, year2) * EMIN(node_active)
- eneprice(node_active, 'rc_therm', year2)/1000 * labor(node_active, year2) * EMIN(node_active) 
- eneprice(node_active, 'transport', year2)/1000 * labor(node_active, year2) * EMIN(node_active) 
- (((alpha(node_active) + beta_rc_spec(node_active) + beta_rc_therm(node_active) + beta_transport(node_active))/alpha(node_active))) * C(node_active, year2)
)) 
;
...
Y(node_active, year) =E=
AE(node_active, year) * (LAKL(node_active) * K(node_active, year)**(rho(node_active) * kpvs(node_active)) * labor(node_active, year)**(rho(node_active) * (1 - kpvs(node_active)))
...
Y(node_active, year)**(1-rho(node_active)) * AE(node_active, year)**rho(node_active) 
...
=E=
INTEREST(node_active, year) + depr(node_active)
;
