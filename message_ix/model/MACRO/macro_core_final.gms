...
SUM(year2$( seq_period(year2,year) ), K(node_active, year2) * (1 - depr(node_active))**duration_period(year2) + duration_period(year2) * I(node_active, year2))
; 
...
* Previous exact formulation kept for reference:
MARGINAL_PRODUCT_CAPITAL(node_active, year) $ (NOT macro_base_period(year))..
...
*EQ_CON(node_active, year) $ (NOT macro_base_period(year))..
*C(node_active, year) =E=
*    SUM(year2$( seq_period(year2,year) ),
*        C(node_active, year2)
*        * ((1 - depr(node_active))**duration_period(year) + ((1 + INTEREST(node_active, year))**duration_period(year) - 1))
*        * (udf(node_active, year) / udf(node_active, year2))
*        * (duration_period(year2) / duration_period(year))
*    )
*;
