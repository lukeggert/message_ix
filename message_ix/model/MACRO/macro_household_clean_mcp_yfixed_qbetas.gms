$TITLE MACRO Household Clean MCP With Fixed Y Path - Quantile-Specific Utility Shares
$EOLCOM #
...
$IF NOT SET out $SETGLOBAL out "output/MacroHouseholdCleanMCP_QBetas.gdx"
$IF NOT SET region $SETGLOBAL region "R12_AFR"


* gams /home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/MACRO/macro_household_clean_mcp_yfixed_qbetas.gms --in="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro_price_10_loob_eneprice.gdx" --stage1="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro_price_10_loob_eneprice.gdx" --out="/home/lukas/household_clean_mcp_qbetas_price_0605.gdx" --region="R12_WEU"
* gams /home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/MACRO/macro_household_clean_mcp_yfixed_qbetas.gms --in="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro.gdx" --stage1="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro.gdx" --out="/home/lukas/household_clean_mcp_qbetas_0605.gdx" --region="R12_WEU"
* gams /home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/MACRO/macro_household_clean_mcp_yfixed_qbetas.gms --in="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro.gdx" --stage1="/home/lukas/environments/macro_uba/lib/python3.10/site-packages/message_ix/model/output/MsgOutput_MESSAGEix_ssp2_baseline_2304_add_macro.gdx" --out="/home/lukas/household_clean_mcp_qbetas_0605_06.gdx" --region="R12_WEU"

* Energy share gradient: D1 (poorest) gets factor (1 + energy_gradient), D10 gets (1 - energy_gradient).
* Linear interpolation in between. Default 0.3 means +/-30 percentage points on energy shares.
* alpha_q + sum(beta_q) = 1 per quantile by construction. Quantile-share-weighted averages
* reproduce the aggregate alpha/beta values loaded from the GDX.
SCALAR energy_gradient 'Energy share gradient across quantiles (D1 highest, D10 lowest)' / 0.6 / ;
SCALAR capital_gradient 'Exponent multiplier for Pareto capital distribution (>1 = more concentrated than income)' / 2.0 / ;
SCALAR capital_return_premium 'Return premium per decile step pi_r (D1 lowest, D10 highest; arithmetic avg = INTEREST_EQ)' / 0.1 / ;
...
PARAMETERS
    ...
    utility_weight(node)                 'Aggregate household utility weight (= 1 for standard Cobb-Douglas)'
    cons_cost_factor(node)               'Aggregate consumption wedge (utility_weight / alpha)'
...
* --- Capital distribution parameters ---
    quantile_capital_share_raw(quantile) 'Raw Pareto capital share (steeper than income Pareto)'
    quantile_capital_share(quantile)     'Normalised capital share by quantile (sums to 1)'

* --- Decile-specific capital return parameters ---
    interest_factor_raw(quantile)        'Raw factor (1+pi_r)^q before normalisation'
    interest_factor_q(quantile)          'Decile-specific interest multiplier (arithmetic avg = 1)'

* --- Quantile-specific utility share parameters ---
    quantile_energy_bias_raw(quantile)   'Linear energy bias before quantile-share normalisation'
    quantile_energy_bias(quantile)       'Normalised energy bias: quantile_share-weighted avg = 1'
    alpha_q(node, quantile)              'Non-energy utility share by quantile'
    beta_rc_spec_q(node, quantile)       'Residential specific energy share by quantile'
    beta_rc_therm_q(node, quantile)      'Residential thermal energy share by quantile'
    beta_transport_q(node, quantile)     'Transport energy share by quantile'
    cons_cost_factor_q(node, quantile)   'Consumption wedge by quantile (= 1 / alpha_q)'
...
* Capital is more concentrated than income: divide the income Pareto exponent by capital_gradient.
* capital_gradient = 1 reproduces income distribution; > 1 shifts wealth to top deciles (lower exponent = more concentrated).
* Effective wealth Pareto alpha = pareto_alpha / capital_gradient (e.g. 2/2 = 1 for capital_gradient=2).
quantile_capital_share_raw(quantile) =
    (1 - (ORD(quantile) - 1) / CARD(quantile)) ** ((pareto_alpha - 1) / (pareto_alpha * capital_gradient))
    - (1 - ORD(quantile) / CARD(quantile)) ** ((pareto_alpha - 1) / (pareto_alpha * capital_gradient)) ;

quantile_capital_share(quantile) =
    quantile_capital_share_raw(quantile)
    / SUM(quantile2, quantile_capital_share_raw(quantile2)) ;

* Decile-specific interest rate: r(q,t) = CARD(q) * r(t) * (1+pi_r)^ORD(q) / SUM_q2(1+pi_r)^ORD(q2)
* Arithmetic average over the 10 deciles equals r(t) by construction.
interest_factor_raw(quantile) = (1 + capital_return_premium) ** ORD(quantile) ;
interest_factor_q(quantile) = CARD(quantile) * interest_factor_raw(quantile)
    / SUM(quantile2, interest_factor_raw(quantile2)) ;

display quantile_share, quantile_capital_share, interest_factor_q ;
...
display alpha, beta_rc_spec, beta_rc_therm, beta_transport, utility_weight ;

* ---------------------------------------------------------------------------
* Quantile-specific utility shares
*
* Raw bias: linear from (1 + energy_gradient) for D1 down to (1 - energy_gradient) for D10.
*   D1:  1 + energy_gradient
*   D10: 1 - energy_gradient
*   Others: linear interpolation.
*
* Normalisation: divide by the quantile_share-weighted mean so that
*   SUM(q, quantile_share(q) * quantile_energy_bias(q)) = 1.
* This guarantees that the quantile_share-weighted average of beta_x_q equals
* beta_x(node_macro) for every energy sector x, and likewise for alpha_q.
*
* The constraint alpha_q + beta_rc_spec_q + beta_rc_therm_q + beta_transport_q = 1
* holds by construction for every quantile (assuming utility_weight = 1 as in
* standard Cobb-Douglas MACRO, i.e. all shares loaded from the GDX sum to 1).
* ---------------------------------------------------------------------------

quantile_energy_bias_raw(quantile) =
    1 + energy_gradient * (CARD(quantile) + 1 - 2 * ORD(quantile)) / (CARD(quantile) - 1) ;

quantile_energy_bias(quantile) =
    quantile_energy_bias_raw(quantile)
    / SUM(quantile2, quantile_share(quantile2) * quantile_energy_bias_raw(quantile2)) ;

beta_rc_spec_q(node_macro, quantile)  = beta_rc_spec(node_macro)  * quantile_energy_bias(quantile) ;
beta_rc_therm_q(node_macro, quantile) = beta_rc_therm(node_macro) * quantile_energy_bias(quantile) ;
beta_transport_q(node_macro, quantile)= beta_transport(node_macro)* quantile_energy_bias(quantile) ;

alpha_q(node_macro, quantile) =
    1
    - beta_rc_spec_q(node_macro, quantile)
    - beta_rc_therm_q(node_macro, quantile)
    - beta_transport_q(node_macro, quantile) ;

* cons_cost_factor_q = utility_weight_q / alpha_q = 1 / alpha_q
* (utility_weight_q = 1 because all four shares sum to 1 by construction)
cons_cost_factor_q(node_macro, quantile) = 1 / alpha_q(node_macro, quantile) ;
...
KAP_HH.LO(node_macro, macro_horizon, quantile) = lotol(node_macro) * k0(node_macro) * quantile_capital_share(quantile) ;
...
KAP_HH.L(node_macro, macro_horizon, quantile) = MAX(lotol(node_macro) * k0(node_macro) * quantile_capital_share(quantile), k_ref(node_macro, macro_horizon) * quantile_capital_share(quantile)) ;
...
* Initialise MU_CAPITAL consistent with FOC_CON being (near) active:
*   MU(t+1,q) = hh_scale * udf(t) / (ccf_q * CON(t,q))
* Using c_ref(t)*qs as the CON proxy keeps initial FOC_CON residuals small
* because c_ref already lies near the optimum (it comes from a prior solve).
MU_CAPITAL.L(node_macro, macro_horizon, quantile)$(NOT macro_base_period(macro_horizon)
        AND c_ref(node_macro, macro_horizon) > 0 AND quantile_share(quantile) > 0) =
    hh_scale * udf(node_macro, macro_horizon)
    / (cons_cost_factor_q(node_macro, quantile) * c_ref(node_macro, macro_horizon) * quantile_share(quantile)) ;

KAP_HH.FX(node_macro, macro_base_period, quantile) = k0(node_macro) * quantile_capital_share(quantile) ;
...
FIRM_FOC_LABOR(node_macro, year)$(macro_horizon(year) AND NOT macro_base_period(year))..
    WAGE_EQ(node_macro, year) =G=
        ...
        * cons_cost_factor_q(node_macro, quantile) replaces the aggregate cons_cost_factor(node_macro)
        * in all household equations, linking budget accounting to the quantile-specific alpha_q.
...
CAPITAL_HH(node_macro, year, quantile)$(macro_horizon(year) AND NOT macro_base_period(year))..
    KAP_HH(node_macro, year, quantile) =E=
            SUM(year2$seq_period(year2, year),
                ...
                * (((1 + interest_factor_q(quantile) * INTEREST_EQ(node_macro, year2)) ** duration_period(year2) - 1) / duration_period(year2))
                + labor_q_exo(node_macro, year2, quantile) * WAGE_EQ(node_macro, year2)
                - energy_floor_cost_exo(node_macro, year2, quantile)
                - cons_cost_factor_q(node_macro, quantile) * CON_HH(node_macro, year2, quantile)
                 )
) ;
                    
TERMINAL_HH(node_macro, last_period, quantile)..
    KAP_HH(node_macro, last_period, quantile)
    * ((((1 + interest_factor_q(quantile) * INTEREST_EQ(node_macro, last_period)) ** duration_period(last_period) - 1) / duration_period(last_period))
    ...
    - cons_cost_factor_q(node_macro, quantile) * CON_HH(node_macro, last_period, quantile)
=E= 0 ;

* utility_weight_q = 1 for all quantiles (alpha_q + sum(beta_q) = 1 by construction),
* so the marginal utility of CON_HH is hh_scale * udf * 1 / CON_HH.
FOC_CON(node_macro, year, quantile)$(macro_horizon(year) AND NOT macro_base_period(year))..
    hh_scale * udf(node_macro, year) * duration_period(year)
        / CON_HH(node_macro, year, quantile)
    - SUM(year2$seq_period(year, year2),
        MU_CAPITAL(node_macro, year2, quantile) * duration_period(year2) * cons_cost_factor_q(node_macro, quantile)
    )
    - NU_TERM(node_macro, year, quantile)$last_period(year) * cons_cost_factor_q(node_macro, quantile)
    =G= 0 ;
    ...
    - SUM(year2$seq_period(year, year2),
        MU_CAPITAL(node_macro, year2, quantile)
            * ((1 - depr(node_macro)) ** duration_period(year2)
            + duration_period(year2) * (((1 + interest_factor_q(quantile) * INTEREST_EQ(node_macro, year)) ** duration_period(year2) - 1) / duration_period(year2)))
    )
    - NU_TERM(node_macro, year, quantile)$last_period(year)
        * ((((1 + interest_factor_q(quantile) * INTEREST_EQ(node_macro, year)) ** duration_period(year) - 1) / duration_period(year))
    ...
    SUM(node_macro,
        hh_scale * (
            SUM((year, quantile)$(macro_horizon(year) AND NOT macro_base_period(year) AND NOT last_period(year)),
                udf(node_macro, year) * (
                    LOG(CON_HH(node_macro, year, quantile))
                    - beta_rc_spec_q(node_macro, quantile) * LOG(eneprice(node_macro, 'rc_spec', year) / 1000)
                    + beta_rc_spec_q(node_macro, quantile) * LOG(beta_rc_spec_q(node_macro, quantile) / alpha_q(node_macro, quantile))
                    - beta_rc_therm_q(node_macro, quantile) * LOG(eneprice(node_macro, 'rc_therm', year) / 1000)
                    + beta_rc_therm_q(node_macro, quantile) * LOG(beta_rc_therm_q(node_macro, quantile) / alpha_q(node_macro, quantile))
                    - beta_transport_q(node_macro, quantile) * LOG(eneprice(node_macro, 'transport', year) / 1000)
...
* Using quantile-specific beta_q / alpha_q for the discretionary energy demand.
E(quantile, node_macro, 'rc_spec', year)$(macro_horizon(year) AND NOT macro_base_period(year)) =
    EMIN(node_macro)
    + (beta_rc_spec_q(node_macro, quantile) / alpha_q(node_macro, quantile))
...
E(quantile, node_macro, 'rc_therm', year)$(macro_horizon(year) AND NOT macro_base_period(year)) =
    EMIN(node_macro)
    + (beta_rc_therm_q(node_macro, quantile) / alpha_q(node_macro, quantile))
    * CON_HH.L(node_macro, year, quantile)
...
E(quantile, node_macro, 'transport', year)$(macro_horizon(year) AND NOT macro_base_period(year)) =
    EMIN(node_macro)
    + (beta_transport_q(node_macro, quantile) / alpha_q(node_macro, quantile))
    * CON_HH.L(node_macro, year, quantile)
...
execute_unload '%out%',
    ...
    quantile_share,
    quantile_capital_share,
    interest_factor_q,
    quantile_energy_bias,
    alpha_q,
    beta_rc_spec_q,
    beta_rc_therm_q,
    beta_transport_q,
