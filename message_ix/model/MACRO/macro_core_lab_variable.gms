...
I(node_active, year) =E=
    ...
    + lab(node_active, year) * WAGE(node_active, year) 
    - eneprice(node_active, 'rc_spec', year)/1000 * lab(node_active, year) * EMIN(node_active)
    - eneprice(node_active, 'rc_therm', year)/1000 * lab(node_active, year) * EMIN(node_active) 
    - eneprice(node_active, 'transport', year)/1000 * lab(node_active, year) * EMIN(node_active) 
...
Y(node_active, year) =E=
    (LAKL(node_active) * K(node_active, year)**(rho(node_active) * kpvs(node_active)) * lab(node_active, year)**(rho(node_active) * (1 - kpvs(node_active)))
...
INTEREST(node_active, year) =E= 
    ...
    * lab(node_active, year)**(rho(node_active)*(1-kpvs(node_active)))
;
...
WAGE(node_active, year) =E=
    ...
    * lab(node_active, year)**(rho(node_active)*(1-kpvs(node_active)) - 1)
;
...
( (lab(node_active, year) * EMIN(node_active) 
