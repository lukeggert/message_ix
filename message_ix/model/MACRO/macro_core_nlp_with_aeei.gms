...
*NEW_ENERGY(node, sector, year_all)
    ...
    EQ_LAB(node, year_all, quantile)
*EQ_CON(node, year_all, quantile)
    EQ_KAP(node, year_all, quantile)
*EQ_DLE_SUM(node, sector, year_all)

    CONSUMPTION_SUM(node, year_all)
    CAPITAL_SUM(node, year_all)
*EQ_KGROW_STEADY(node, year_all)
    EQ_KGROW_STEADY2(node, year_all)

    YE_GROWTH_CAP(node, sector, year_all)     Upper bound on YE growth relative to previous period
;
...
C(node_active, year) + I(node_active, year) + EC(node_active, year) + EC_REG(node_active, year)
;
...
SUM(year2$( seq_period(year2,year) ), K(node_active, year2) * (1 - depr(node_active))**duration_period(year) + duration_period(year) * I(node_active, year))
; 

INVESTMENT(node_active, year) $ (NOT macro_base_period(year) AND NOT last_period(year))..
I(node_active, year) =E=
    ...
    - eneprice(node_active, 'rc_spec', year) * aeei_factor(node_active, 'rc_spec', year)/1000 * e_min(node_active, 'rc_spec')
    - eneprice(node_active, 'rc_therm', year) * aeei_factor(node_active, 'rc_therm', year)/1000 * e_min(node_active, 'rc_therm')
    - eneprice(node_active, 'transport', year) * aeei_factor(node_active, 'transport', year)/1000 * e_min(node_active, 'transport')
...
Y(node_active, year) =E=
    (ACONST(node_active) * K(node_active, year)**(rho(node_active) * kpvs(node_active)) * labor(node_active, year)**(rho(node_active) * (1 - kpvs(node_active)))
...
E(node_active, sector, year) =E=
        ...
        + (beta_rc_spec(node_active) / alpha(node_active)) * C(node_active, year) / (eneprice(node_active, 'rc_spec', year) * aeei_factor(node_active, 'rc_spec', year)/1000)
        ) $ sameas(sector, 'rc_spec')

    + ( e_min(node_active, 'rc_therm')
        + (beta_rc_therm(node_active) / alpha(node_active)) * C(node_active, year) / (eneprice(node_active, 'rc_therm', year) * aeei_factor(node_active, 'rc_therm', year)/1000)
        ) $ sameas(sector, 'rc_therm')

    + ( e_min(node_active, 'transport')
        + (beta_transport(node_active) / alpha(node_active)) * C(node_active, year) / (eneprice(node_active, 'transport', year) * aeei_factor(node_active, 'transport', year)/1000)
...
E_HH(node_active, sector, year, quantile) =E=
        ...
        + (beta_rc_spec(node_active) / alpha(node_active)) * CON(node_active, year, quantile) / (eneprice(node_active, 'rc_spec', year) * aeei_factor(node_active, 'rc_spec', year)/1000)
        ) $ sameas(sector, 'rc_spec')

    + ( 0.2 * e_min(node_active, 'rc_therm')
        + (beta_rc_therm(node_active) / alpha(node_active)) * CON(node_active, year, quantile) / (eneprice(node_active, 'rc_therm', year) * aeei_factor(node_active, 'rc_therm', year)/1000)
        ) $ sameas(sector, 'rc_therm')

    + ( 0.2 * e_min(node_active, 'transport')
        + (beta_transport(node_active) / alpha(node_active)) * CON(node_active, year, quantile) / (eneprice(node_active, 'transport', year) * aeei_factor(node_active, 'transport', year)/1000)
        ) $ sameas(sector, 'transport')
...
*YE(node_active, sector, year) =E=
*SUM(year2$( seq_period(year2,year) ), YE(node_active, sector, year2) * (1 - depr(node_active))) + NEWENE(node_active, sector, year)
*;
...
COST_ENERGY_REG(node_active, year) $ (NOT macro_base_period(year))..
EC_REG(node_active, year) =E=
SUM(sector, q_newte(node_active, sector, year) * SQR(YE(node_active, sector, year)))
;
...
** Steady state: constant capital growth rate in the last period
EQ_KGROW_STEADY2(node_active, last_period)..
KGROW(node_active, last_period) =E= KGROW(node_active, last_period)
;

TERMINAL_CONDITION(node_active, last_period)..
I(node_active, last_period) =E= SUM(year2$seq_period(year2, last_period), K(node_active, last_period) * (KGROW(node_active, last_period) + depr(node_active)))
;

*** Steady state: constant capital growth rate in the last period
*EQ_KGROW_STEADY(node_active, last_period)..
*KGROW(node_active, last_period) =E= SUM(year2$seq_period(year2, last_period), KGROW(node_active, year2))
*;
...
* Aggregate consumption and capital
CONSUMPTION_SUM(node_active, year) $ (NOT macro_base_period(year))..
C(node_active, year) =E= SUM(quantile, CON(node_active, year, quantile))
;

CAPITAL_SUM(node_active, year) $ (NOT macro_base_period(year))..
K(node_active, year) =E= SUM(quantile, KAP(node_active, year, quantile))
;

* Household labor, consumption and capital equations by quantile
...
KAP(node_active, year, quantile) =E=
    SUM(year2$( seq_period(year2,year) ),
        ...
        duration_period(year) * (
            ...
            - eneprice(node_active, 'rc_spec', year) * aeei_factor(node_active, 'rc_spec', year)/1000 * quantile_share(quantile) * e_min(node_active, 'rc_spec')
            - eneprice(node_active, 'rc_therm', year) * aeei_factor(node_active, 'rc_therm', year)/1000 * quantile_share(quantile) * e_min(node_active, 'rc_therm') 
            - eneprice(node_active, 'transport', year) * aeei_factor(node_active, 'transport', year)/1000 * quantile_share(quantile) * e_min(node_active, 'transport') 
...
*EQ_DLE_SUM(node_active, sector, year) $ (
*    NOT macro_base_period(year)
*    AND (sameas(sector, 'rc_spec') OR sameas(sector, 'rc_therm') OR sameas(sector, 'transport'))
*)..
*E(node_active, sector, year) =E= SUM(quantile, E_Q(node_active, sector, year, quantile))
*;

***
* Equation YE_GROWTH_CAP
* ---------------------------------
* Limits the growth of productive energy YE in each period relative to the
* actual (endogenous) level of the previous period.  This replaces the
* pre-solve .UP bound in macro_solve.gms, which used start-value levels and
* therefore could not properly track the optimal trajectory.
*
* .. math:: \text{YE}_{n,s,t} \leq \text{newene\_share\_cap}_{n,s,t} \cdot \text{YE}_{n,s,t-1}
*
***

YE_GROWTH_CAP(node_active, sector, year) $ (NOT macro_base_period(year))..
YE(node_active, sector, year) =L=
    SUM(year2$(seq_period(year2, year)),
        newene_share_cap(node_active, sector, year) * YE(node_active, sector, year2))
;


$ontext
* Investitionsvariable und exakte Kapitaldynamik pro Haushalt
POSITIVE VARIABLES
    I_HH(node_active, year, quantile)   'Investment by household and period';

EQUATIONS
    CAPITAL_HH(node_active, year, quantile)   'Household capital accumulation';
    INVESTMENT_HH(node_active, year, quantile) 'Household investment accounting';

* Exakte Kapitaldynamik pro Haushalt (wie Aggregat, aber für jedes quantile)
CAPITAL_HH(node_active, year, quantile) $ (NOT macro_base_period(year))..
KAP(node_active, year, quantile) =E=
    SUM(year2$( seq_period(year2,year) ),
        KAP(node_active, year2, quantile) * (1 - depr(node_active))**duration_period(year)
...
* Investitionsgleichung pro Haushalt (exakt wie Aggregat, aber für jedes quantile)
INVESTMENT_HH(node_active, year, quantile) $ (NOT macro_base_period(year) AND NOT last_period(year))..
I_HH(node_active, year, quantile) =E=
    SUM(year2$( seq_period(year2,year) ),
        (KAP(node_active, year2, quantile) * ((1 + INTEREST(node_active, year))**duration_period(year) - 1) / duration_period(year))
        + LAB(node_active, year, quantile) * WAGE(node_active, year)
        - eneprice(node_active, 'rc_spec', year) * aeei_factor(node_active, 'rc_spec', year)/1000 * quantile_share(quantile) * EMIN(node_active)
        - eneprice(node_active, 'rc_therm', year) * aeei_factor(node_active, 'rc_therm', year)/1000 * quantile_share(quantile) * EMIN(node_active)
        - eneprice(node_active, 'transport', year) * aeei_factor(node_active, 'transport', year)/1000 * quantile_share(quantile) * EMIN(node_active)
        - ((alpha(node_active) + beta_rc_spec(node_active) + beta_rc_therm(node_active) + beta_transport(node_active))/alpha(node_active)) * CON(node_active, year, quantile)
    )
;
$offtext
