...
VARIABLES
    ...
    EC_REG(node, year_all)           Regularization costs for new end-use energy expansion
...
SUM(node_active,
    1000 * (SUM(year $ (NOT macro_base_period(year) AND NOT last_period(year)),
            udf(node_active, year) * SUM(quantile,
            ( (alpha_q(node_active, quantile) + beta_rc_spec_q(node_active, quantile) + beta_rc_therm_q(node_active, quantile) + beta_transport_q(node_active, quantile)) * LOG(CON(node_active, year, quantile)) 
            - beta_rc_spec_q(node_active, quantile) * LOG(eneprice(node_active, 'rc_spec', year)/1000) + beta_rc_spec_q(node_active, quantile) * LOG(beta_rc_spec_q(node_active, quantile)/alpha_q(node_active, quantile)) 
            - beta_rc_therm_q(node_active, quantile) * LOG(eneprice(node_active, 'rc_therm', year)/1000) + beta_rc_therm_q(node_active, quantile) * LOG(beta_rc_therm_q(node_active, quantile)/alpha_q(node_active, quantile))
            - beta_transport_q(node_active, quantile) * LOG(eneprice(node_active, 'transport', year)/1000) + beta_transport_q(node_active, quantile) * LOG(beta_transport_q(node_active, quantile)/alpha_q(node_active, quantile)) ) ) * duration_period(year) )
        + SUM(year $ last_period(year),
            udf(node_active, year) * SUM(quantile,
            ( (alpha_q(node_active, quantile) + beta_rc_spec_q(node_active, quantile) + beta_rc_therm_q(node_active, quantile) + beta_transport_q(node_active, quantile)) * LOG(CON(node_active, year, quantile)) 
            - beta_rc_spec_q(node_active, quantile) * LOG(eneprice(node_active, 'rc_spec', year)/1000) + beta_rc_spec_q(node_active, quantile) * LOG(beta_rc_spec_q(node_active, quantile)/alpha_q(node_active, quantile)) 
            - beta_rc_therm_q(node_active, quantile) * LOG(eneprice(node_active, 'rc_therm', year)/1000) + beta_rc_therm_q(node_active, quantile) * LOG(beta_rc_therm_q(node_active, quantile)/alpha_q(node_active, quantile))
            - beta_transport_q(node_active, quantile) * LOG(eneprice(node_active, 'transport', year)/1000) + beta_transport_q(node_active, quantile) * LOG(beta_transport_q(node_active, quantile)/alpha_q(node_active, quantile)) ) ) * (duration_period(year) ) + 1/finite_time_corr(node_active, year)) )
)
;
...
TE(node_active, sector, year) =E=
SUM(year2$( seq_period(year2,year) ), TE(node_active, sector, year2)) * (1 - depr(node_active))**duration_period(year) + NEWENE(node_active, sector, year)
;

ENERGY_SUPPLY(node_active, sector, year) $ (NOT macro_base_period(year))..
PHYSENE(node_active, sector, year) =E=
TE(node_active, sector, year) * aeei_factor(node_active, sector, year)
;

* old slack formulation for quick switch
*ENERGY_SUPPLY(node_active, sector, year) $ (NOT macro_base_period(year))..
...
SUM(sector, q_newte(node_active, sector, year) * SQR(NEWENE(node_active, sector, year)))
;
...
KGROW(node_active, last_period) =E= grow(node_active, last_period)
;

*$ontext
TERMINAL_CONDITION(node_active, last_period)..
I(node_active, last_period) =E= K(node_active, last_period) * (KGROW(node_active, last_period) + depr(node_active))
;
...
KAP(node_active, year, quantile) =E=
    SUM(year2$( seq_period(year2,year) ),
        ...
        duration_period(year) * (
            (KAP(node_active, year2, quantile) * ((1 + INTEREST(node_active, year2))**duration_period(year2) - 1) / duration_period(year2))
            + LAB(node_active, year2, quantile) * WAGE(node_active, year2) 
            - eneprice(node_active, 'rc_spec', year2)/1000 * LAB(node_active, year2, quantile) * EMIN(node_active)
            - eneprice(node_active, 'rc_therm', year2)/1000 * LAB(node_active, year2, quantile) * EMIN(node_active) 
            - eneprice(node_active, 'transport', year2)/1000 * LAB(node_active, year2, quantile) * EMIN(node_active) 
            - ((alpha_q(node_active, quantile) + beta_rc_spec_q(node_active, quantile) + beta_rc_therm_q(node_active, quantile) + beta_transport_q(node_active, quantile))/alpha_q(node_active, quantile)) * CON(node_active, year2, quantile)
            ))
;
